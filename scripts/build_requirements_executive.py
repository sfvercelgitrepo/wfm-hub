...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .cap-map {{ width: 100%; }}
    .cap-map-row {{ display: flex; width: 100%; align-items: stretch; }}
    .cap-map-seg {{
      flex: 1 1 0; min-width: 0; padding: 0 5px;
      border-right: 1px solid rgba(74,93,120,.35);
      display: flex; flex-direction: column; align-items: stretch;
    }}
    .cap-map-seg:last-child {{ border-right: none; }}
    .cap-map-bridge {{ height: 22px; }}
    .cap-map-bridge-features {{ height: 16px; }}
    .cap-map-bridge .cap-map-seg {{
      position: relative; display: flex; justify-content: center; align-items: flex-start;
    }}
    .cap-map-bridge .cap-map-seg::before {{
      content: ""; width: 2px; height: 22px; background: #4a5d78;
    }}
    .cap-map-bridge-features .cap-map-seg::after {{
      content: ""; position: absolute; top: 0; left: 8%; right: 8%; height: 2px; background: #4a5d78;
    }}
    .cap-map-bridge-features .cap-map-subcol::before {{
      content: ""; width: 2px; height: 14px; background: #4a5d78; margin: 0 auto;
    }}
    .cap-map-layer-label {{
      font-size: 9px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase;
      color: var(--muted); text-align: center; margin: 0 0 8px;
    }}
    .cap-map-nodes {{ display: flex; gap: 6px; align-items: stretch; width: 100%; }}
    .cap-map-node-slot {{ flex: 1 1 0; min-width: 0; display: flex; flex-direction: column; align-items: center; }}
    .cap-map-subcols {{ display: flex; gap: 6px; width: 100%; align-items: flex-start; }}
    .cap-map-subcol {{ flex: 1 1 0; min-width: 0; display: flex; flex-direction: column; align-items: center; }}
    .cap-map-epic-stack {{ display: flex; flex-direction: column; gap: 6px; width: 100%; }}
    .cap-map-card {{
...
<body>
  ...
  <div class="page">
    ...
    <div class="exec-panel">
      <div class="exec-head">
        ...
        <div class="exec-sub">All capabilities in one row, features on the next level, epics below · full tree visible · fit to width</div>
      </div>
  ...
  <script>
    ...
    function renderExecutive(filteredRows) {{
      ...
      html += '<div class="cap-map-layer-label">Capabilities</div>';
      html += '<div class="cap-map-row">';
      roots.forEach(function (cap) {{
        html += '<div class="cap-map-seg">' + renderCapCard(cap, counts, "capability") + '</div>';
      }});
      html += '</div>';

      html += '<div class="cap-map-row cap-map-bridge">';
      roots.forEach(function () {{ html += '<div class="cap-map-seg"></div>'; }});
      html += '</div>';

      html += '<div class="cap-map-layer-label">Features</div>';
      html += '<div class="cap-map-row">';
      roots.forEach(function (cap) {{
        const features = cap.children || [];
        html += '<div class="cap-map-seg"><div class="cap-map-nodes">';
        if (!features.length) {{
          html += '<div class="cap-map-node-slot"><div class="cap-map-card feature"><div class="cap-map-type">Feature</div><div class="cap-map-title">—</div></div></div>';
        }} else {{
          features.forEach(function (feat) {{
            html += '<div class="cap-map-node-slot">' + renderCapCard(feat, counts, "feature") + '</div>';
          }});
        }}
        html += '</div></div>';
      }});
      ...
      html += '<div class="cap-map-row cap-map-bridge cap-map-bridge-features">';
      roots.forEach(function (cap) {{
        const features = cap.children || [];
        html += '<div class="cap-map-seg"><div class="cap-map-subcols">';
        if (!features.length) {{
          html += '<div class="cap-map-subcol"></div>';
        }} else {{
          features.forEach(function () {{ html += '<div class="cap-map-subcol"></div>'; }});
        }}
        html += '</div></div>';
      }});
      html += '</div>';

      html += '<div class="cap-map-layer-label">Epics</div>';
      html += '<div class="cap-map-row">';
      roots.forEach(function (cap) {{
        const features = cap.children || [];
        html += '<div class="cap-map-seg"><div class="cap-map-subcols">';
        if (!features.length) {{
          html += '<div class="cap-map-subcol"><div class="cap-map-epic-stack"></div></div>';
        }} else {{
          features.forEach(function (feat) {{
            const epics = epicLevelNodes(feat);
            html += '<div class="cap-map-subcol"><div class="cap-map-epic-stack">';
